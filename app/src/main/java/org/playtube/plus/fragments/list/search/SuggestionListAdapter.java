...
public class SuggestionListAdapter extends RecyclerView.Adapter<SuggestionListAdapter.SuggestionItemHolder> {
    ...
    public SuggestionItemHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new SuggestionItemHolder(LayoutInflater.from(context).inflate(org.playtube.plus.R.layout.item_search_suggestion, parent, false));
    }
    ...
    public static class SuggestionItemHolder extends RecyclerView.ViewHolder {
        ...
        private SuggestionItemHolder(View rootView) {
            super(rootView);
            suggestionIcon = rootView.findViewById(org.playtube.plus.R.id.item_suggestion_icon);
            itemSuggestionQuery = rootView.findViewById(org.playtube.plus.R.id.item_suggestion_query);

            queryView = rootView.findViewById(org.playtube.plus.R.id.suggestion_search);
            insertView = rootView.findViewById(org.playtube.plus.R.id.suggestion_insert);

            historyResId = resolveResourceIdFromAttr(rootView.getContext(), org.playtube.plus.R.attr.history);
            searchResId = resolveResourceIdFromAttr(rootView.getContext(), org.playtube.plus.R.attr.search);
        }

        private void updateFrom(SuggestionItem item) {
            suggestionIcon.setImageResource(item.fromHistory ? historyResId : org.playtube.plus.R.drawable.ic_search_60_24dp);
