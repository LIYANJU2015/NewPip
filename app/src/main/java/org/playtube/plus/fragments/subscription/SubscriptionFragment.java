package org.playtube.plus.fragments.subscription;
...
import org.playtube.plus.subscription.services.SubscriptionsExportService;
...
public class SubscriptionFragment extends BaseStateFragment<List<SubscriptionEntity>> {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(org.playtube.plus.R.layout.fragment_subscription, container, false);
    }
    ...
    private View addItemView(final String title, @DrawableRes final int icon, ViewGroup container) {
        final View itemRoot = View.inflate(getContext(), org.playtube.plus.R.layout.subscription_import_export_item, null);
    ...
    private void setupImportFromItems(final ViewGroup listHolder) {
        final View previousBackupItem = addItemView(getString(org.playtube.plus.R.string.previous_export), org.playtube.plus.R.drawable.ic_backup_black_24dp, listHolder);
        previousBackupItem.setOnClickListener(item -> onImportPreviousSelected());

        final String[] services = getResources().getStringArray(org.playtube.plus.R.array.service_list);
...
//                iconView.setColorFilter(iconColor, PorterDuff.Mode.SRC_IN);
    ...
    private void setupExportToItems(final ViewGroup listHolder) {
        final View previousBackupItem = addItemView(getString(org.playtube.plus.R.string.file), org.playtube.plus.R.drawable.ic_save_black_24dp, listHolder);
        ...
        if (data != null && data.getData() != null && resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_EXPORT_CODE) {
                ...
                if (!exportFile.getParentFile().canWrite() || !exportFile.getParentFile().canRead()) {
                    Toast.makeText(activity, org.playtube.plus.R.string.invalid_directory, Toast.LENGTH_SHORT).show();
                } else {
    ...
    protected void initViews(View rootView, Bundle savedInstanceState) {
        ...
        itemsList = rootView.findViewById(org.playtube.plus.R.id.items_list);
        itemsList.setLayoutManager(new MyLinearLayoutManager(activity));

        infoListAdapter.setHeader(headerRootLayout = activity.getLayoutInflater().inflate(org.playtube.plus.R.layout.subscription_header, itemsList, false));
        whatsNewItemListHeader = headerRootLayout.findViewById(org.playtube.plus.R.id.whats_new);
        importExportListHeader = headerRootLayout.findViewById(org.playtube.plus.R.id.import_export);
        importExportOptions = headerRootLayout.findViewById(org.playtube.plus.R.id.import_export_options);

        infoListAdapter.useMiniItemVariants(true);
        itemsList.setAdapter(infoListAdapter);

        setupImportFromItems(headerRootLayout.findViewById(org.playtube.plus.R.id.import_from_options));
        setupExportToItems(headerRootLayout.findViewById(org.playtube.plus.R.id.export_to_options));
        ...
        importExportOptions.addListener(getExpandIconSyncListener(headerRootLayout.findViewById(org.playtube.plus.R.id.import_export_expand_icon)));
    ...
    protected boolean onError(Throwable exception) {
        ...
        onUnrecoverableError(exception, UserAction.SOMETHING_ELSE, "none", "Subscriptions", org.playtube.plus.R.string.general_error);
        return true;
    }
}
