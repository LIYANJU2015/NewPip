...
public class FBAdUtils {
    ...
    private void showDialog(View view, Activity activity) {
        if (view != null && !activity.isFinishing()) {
            try {
                ...
                dialog.show();
                view.findViewById(org.tubeplayer.plus.R.id.ad_close_iv).setOnClickListener(new View.OnClickListener() {
                    @Override
    ...
    private View setupAdView(NativeAd nativeAd) {
        try {
            View currentAdView = LayoutInflater.from(sContext)
                    .inflate(org.tubeplayer.plus.R.layout.fb_big_ad_layout, null);
            MediaView nativeAdMedia = currentAdView.findViewById(org.tubeplayer.plus.R.id.fb_half_mv_view);
            FrameLayout adChoicesFrame = currentAdView.findViewById(org.tubeplayer.plus.R.id.fb_adChoices_view);
            ImageView nativeAdIcon = currentAdView.findViewById(org.tubeplayer.plus.R.id.fb_half_icon_iv);
            TextView nativeAdTitle = currentAdView.findViewById(org.tubeplayer.plus.R.id.fb_banner_title_tv);
            TextView nativeAdBody = currentAdView.findViewById(org.tubeplayer.plus.R.id.fb_banner_desc_tv);
            TextView nativeAdCallToAction = currentAdView.findViewById(org.tubeplayer.plus.R.id.fb_half_actionbtn);
            currentAdView.findViewById(org.tubeplayer.plus.R.id.fb_half_iv).setVisibility(View.GONE);
    ...
    public View setUpItemNativeAdView(Activity activity, NativeAd nativeAd, boolean isSmallItem) {
        ...
        View adView = LayoutInflater.from(activity).inflate(org.tubeplayer.plus.R.layout.fb_ad_list_item, null);
        FrameLayout imageAdFrame = adView.findViewById(org.tubeplayer.plus.R.id.image_ad_frame);

        ImageView nativeAdIcon;
        if (isSmallItem) {
            imageAdFrame.setVisibility(View.GONE);
            nativeAdIcon = adView.findViewById(org.tubeplayer.plus.R.id.image_ad2);
            nativeAdIcon.setVisibility(View.VISIBLE);
        } else {
            nativeAdIcon = adView.findViewById(org.tubeplayer.plus.R.id.image_ad);
            ...
            adView.findViewById(org.tubeplayer.plus.R.id.image_ad2).setVisibility(View.GONE);
        }

        FrameLayout adChoicesFrame = adView.findViewById(org.tubeplayer.plus.R.id.fb_adChoices2);
        TextView nativeAdTitle = adView.findViewById(org.tubeplayer.plus.R.id.title);
        TextView nativeAdBody = adView.findViewById(org.tubeplayer.plus.R.id.text);
        TextView nativeAdCallToAction = adView.findViewById(org.tubeplayer.plus.R.id.call_btn_tv);
