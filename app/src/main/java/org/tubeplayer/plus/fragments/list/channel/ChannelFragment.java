package org.tubeplayer.plus.fragments.list.channel;
...
import org.tubeplayer.plus.fragments.list.BaseListInfoFragment;
import org.tubeplayer.plus.playlist.ChannelPlayQueue;
...
public class ChannelFragment extends BaseListInfoFragment<ChannelInfo> {
    ...
    protected View getListHeader() {
        headerRootLayout = activity.getLayoutInflater().inflate(org.tubeplayer.plus.R.layout.channel_header, itemsList, false);
        headerChannelBanner = headerRootLayout.findViewById(org.tubeplayer.plus.R.id.channel_banner_image);
        headerAvatarView = headerRootLayout.findViewById(org.tubeplayer.plus.R.id.channel_avatar_view);
        headerTitleView = headerRootLayout.findViewById(org.tubeplayer.plus.R.id.channel_title_view);
        headerSubscribersTextView = headerRootLayout.findViewById(org.tubeplayer.plus.R.id.channel_subscriber_view);
        headerSubscribeButton = headerRootLayout.findViewById(org.tubeplayer.plus.R.id.channel_subscribe_button);
        playlistCtrl = headerRootLayout.findViewById(org.tubeplayer.plus.R.id.playlist_control);
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        if(useAsFrontPage && supportActionBar != null) {
            ...
            inflater.inflate(org.tubeplayer.plus.R.menu.menu_channel, menu);

            if (DEBUG) Log.d(TAG, "onCreateOptionsMenu() called with: menu = [" + menu +
                    "], inflater = [" + inflater + "]");
            menuRssButton = menu.findItem(org.tubeplayer.plus.R.id.menu_item_rss);
        }
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case org.tubeplayer.plus.R.id.menu_item_rss:
                openRssFeed();
                break;
    ...
    private void updateSubscription(final ChannelInfo info) {
        ...
        final Consumer<Throwable> onError = new Consumer<Throwable>() {
            ...
            public void accept(@NonNull Throwable throwable) throws Exception {
                onUnrecoverableError(throwable, UserAction.SUBSCRIPTION, NewPipe.getNameOfService(info.getServiceId()), "Updating Subscription for " + info.getUrl(), org.tubeplayer.plus.R.string.subscription_update_failed);
            }
        };
    ...
    private Disposable monitorSubscribeButton(final Button subscribeButton, final Function<Object, Object> action) {
        ...
        final Consumer<Throwable> onError = new Consumer<Throwable>() {
            @Override
            public void accept(@NonNull Throwable throwable) throws Exception {
                onUnrecoverableError(throwable, UserAction.SUBSCRIPTION, NewPipe.getNameOfService(currentInfo.getServiceId()), "Subscription Change", org.tubeplayer.plus.R.string.subscription_change_failed);
            }
        };
    ...
    private void updateSubscribeButton(boolean isSubscribed) {
        ...
        int subscribeBackground = ContextCompat.getColor(activity, org.tubeplayer.plus.R.color.subscribe_background_color);
        int subscribeText = ContextCompat.getColor(activity, org.tubeplayer.plus.R.color.subscribe_text_color);
        int subscribedBackground = ContextCompat.getColor(activity, org.tubeplayer.plus.R.color.subscribed_background_color);
        int subscribedText = ContextCompat.getColor(activity, org.tubeplayer.plus.R.color.subscribed_text_color);

        if (!isSubscribed) {
            headerSubscribeButton.setText(org.tubeplayer.plus.R.string.subscribe_button_title);
            ...
            headerSubscribeButton.setText(org.tubeplayer.plus.R.string.subscribed_button_title);
    ...
    public void handleNextItems(ListExtractor.InfoItemsPage result) {
        ...
        if (!result.getErrors().isEmpty()) {
            showSnackBarError(result.getErrors(), UserAction.REQUESTED_CHANNEL, NewPipe.getNameOfService(serviceId),
                    "Get next page of: " + url, org.tubeplayer.plus.R.string.general_error);
        }
    }
    ...
    protected boolean onError(Throwable exception) {
        ...
        int errorId = exception instanceof ExtractionException ? org.tubeplayer.plus.R.string.parsing_error : org.tubeplayer.plus.R.string.general_error;
