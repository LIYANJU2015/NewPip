...
public class SuggestionListAdapter extends RecyclerView.Adapter<SuggestionListAdapter.SuggestionItemHolder> {
    ...
    public SuggestionItemHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new SuggestionItemHolder(LayoutInflater.from(context).inflate(org.tubeplayer.plus.R.layout.item_search_suggestion, parent, false));
    }
    ...
    public static class SuggestionItemHolder extends RecyclerView.ViewHolder {
        ...
        private SuggestionItemHolder(View rootView) {
            super(rootView);
            suggestionIcon = rootView.findViewById(org.tubeplayer.plus.R.id.item_suggestion_icon);
            itemSuggestionQuery = rootView.findViewById(org.tubeplayer.plus.R.id.item_suggestion_query);

            queryView = rootView.findViewById(org.tubeplayer.plus.R.id.suggestion_search);
            insertView = rootView.findViewById(org.tubeplayer.plus.R.id.suggestion_insert);

            historyResId = resolveResourceIdFromAttr(rootView.getContext(), org.tubeplayer.plus.R.attr.history);
            searchResId = resolveResourceIdFromAttr(rootView.getContext(), org.tubeplayer.plus.R.attr.search);
        }

        private void updateFrom(SuggestionItem item) {
            suggestionIcon.setImageResource(item.fromHistory ? historyResId : org.tubeplayer.plus.R.drawable.ic_search_60_24dp);
