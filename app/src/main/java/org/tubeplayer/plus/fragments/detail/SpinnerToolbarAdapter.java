...
public class SpinnerToolbarAdapter extends BaseAdapter {
    ...
    private View getCustomView(int position, View convertView, ViewGroup parent, boolean isDropdownItem) {
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(org.tubeplayer.plus.R.layout.resolutions_spinner_item, parent, false);
        }
        convertView.setBackground(ContextCompat.getDrawable(context, org.tubeplayer.plus.R.drawable.spin_item_selector));


        ImageView woSoundIcon = convertView.findViewById(org.tubeplayer.plus.R.id.wo_sound_icon);
